...
const initApp = () => {
    ...
    // ... Rest of logic (EventListeners, etc) ...

    /* --- RE-INSERT LISTENERS HERE BECAUSE WE REPLACED THE TOP --- */
    // Note: The replace_file_content tool needs care as I'm replacing lines 1-118 which includes listeners.
    // I need to make sure I don't lose the listeners.
    // Actually, lines 68-76 (typeButtons listener) are inside the block I'm replacing.
    ...
    function navigateToSearch(mode, type, label) {
        try {
            ...
            appTitle.textContent = `【${modeText}】${label}`;
            searchTitle.textContent = '条件設定';
            if (priceLabel) priceLabel.textContent = mode === 'rent' ? 'お家賃' : '価格';
